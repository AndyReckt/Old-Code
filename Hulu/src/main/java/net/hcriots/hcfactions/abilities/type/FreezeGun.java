...
public class FreezeGun extends Ability {
    ...
    public static int SNOWBALL_RADIUS = 10;
    ...
    public void on(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Snowball && e.getEntity() instanceof Player) {
            ...
            UUID uuid = UUID.fromString(snowball.getMetadata("freezegun").get(0).asString());
            Player player = Hulu.getInstance().getServer().getPlayer(uuid);
            Location location = player.getLocation().clone();
            Location swappedLocation = e.getEntity().getLocation().clone();


            if (location.distance(swappedLocation) > 10) {
                player.sendMessage(damaged.getDisplayName() + ChatColor.RED + " is more then " + ChatColor.WHITE + SNOWBALL_RADIUS + ChatColor.RED + " blocks away from you!");
                return;
            }
