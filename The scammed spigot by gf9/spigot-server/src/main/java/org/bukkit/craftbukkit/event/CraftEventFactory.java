    ...
    private static EntityDamageEvent callEntityDamageEvent(Entity damager, Entity damagee, DamageCause cause, Map<DamageModifier, Double> modifiers, Map<DamageModifier, Function<? super Double, Double>> modifierFunctions) {
        ...
        if (damager instanceof Player) {
            ...
            for (org.bukkit.inventory.ItemStack itemStack : player.getInventory().getContents()) {
                if (itemStack.getItemMeta().hasCustomEnchants()) {
                    for (CustomEnchantment enchantment : itemStack.getItemMeta().getCustomEnchants().keySet()) {
                        callEvent(new DamageEntityWithCustomEnchantmentEvent(player, itemStack, enchantment, (EntityDamageByEntityEvent) event));
                    }
                }
            }

            for (org.bukkit.inventory.ItemStack itemStack : player.getInventory().getArmorContents()) {
                if (itemStack.getItemMeta().hasCustomEnchants()) {
                    for (CustomEnchantment enchantment : itemStack.getItemMeta().getCustomEnchants().keySet()) {
                        callEvent(new DamageEntityWithCustomEnchantmentEvent(player, itemStack, enchantment, (EntityDamageByEntityEvent) event));
                    }
