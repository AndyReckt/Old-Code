...
object UnwarnCommand {
    @Command(["unwarn"], permission = "stark.command.unwarn", description = "Unwarn a player", async = true)
    @JvmStatic
    fun execute(sender: CommandSender, @Flag(value = ["s", "silentMode"], description = "Silently unwarn the player") silent: Boolean, @Param("target") target: BukkitProfile, @Param("reason", wildcard = true) reason: String) {
        var issuer: UUID? = null
        if (sender is Player) {
            issuer = sender.uniqueId
        }

        val activePunishment = target.getActivePunishment(ProfilePunishmentType.WARN)
        if (activePunishment == null) {
            sender.sendMessage("${target.getPlayerListName()} ${ChatColor.RED}is not warned.")
            return
        }
