...
import cc.fyre.stark.engine.command.data.argument.Arguments
import cc.fyre.stark.engine.command.data.flag.FlagData
import cc.fyre.stark.engine.command.data.parameter.ParameterData
    ...
    operator fun invoke(sender: CommandSender, arguments: Arguments): Boolean {
        ...
        for (data in this.parameters) {
            if (data is FlagData) {
                ...
                if (type == null) {
                    sender.sendMessage("${ChatColor.RED}No data type found: ${
                        (data.parameterType
        ...
        try {
            ...
            if (!async && logToConsole && stopwatch.elapsedMillis() >= executionThreshold) {
                Stark.instance.logger.warning("Command '/" + getFullLabel() + "' took " + stopwatch.elapsedMillis() + "ms!")
            }
