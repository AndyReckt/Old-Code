...
class Cuboid : Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    constructor(l1: Location, l2: Location = l1) {
        if (!l1.world.equals(l2.world)) {
            throw IllegalArgumentException("Locations must be on the same world")
        }
        this.worldName = l1.world.name
        this.lowerX = Math.min(l1.blockX, l2.blockX)
        this.lowerY = Math.min(l1.blockY, l2.blockY)
        this.lowerZ = Math.min(l1.blockZ, l2.blockZ)
        this.upperX = Math.max(l1.blockX, l2.blockX)
        this.upperY = Math.max(l1.blockY, l2.blockY)
        this.upperZ = Math.max(l1.blockZ, l2.blockZ)
    }
    ...
    operator fun contains(l: Location): Boolean {
        return this.worldName == l.world.name && this.contains(l.blockX, l.blockY, l.blockZ)
    }

    operator fun contains(e: Entity): Boolean {
        return this.contains(e.location)
    }
