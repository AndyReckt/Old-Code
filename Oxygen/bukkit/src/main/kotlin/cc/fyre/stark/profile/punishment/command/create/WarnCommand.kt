...
object WarnCommand {
    @Command(["warn"], permission = "stark.command.warn", description = "Warn a player", async = true)
    @JvmStatic
    fun execute(sender: CommandSender, @Flag(value = ["s", "silentMode"], description = "Silently warn the player") silent: Boolean, @Param("target") target: BukkitProfile, @Param("reason", wildcard = true) reason: String) {
        ...
        if (sender is Player) {
            ...
            if (target.getRank().displayOrder <= senderProfile.getRank().displayOrder) {
                sender.sendMessage("${ChatColor.RED}You can't warn this player.")
                return
            }
        ...
        /*if (target.getActivePunishment(ProfilePunishmentType.WARN) != null) {
            sender.sendMessage("${target.getPlayerListName()} ${ChatColor.RED}is already warned.")
            return
        }*/

        val punishment = ProfilePunishment()
        punishment.uuid = UUID.randomUUID()
        punishment.type = ProfilePunishmentType.WARN
